...
//			this.getSession().flush();
	...
	public List findBySQL(String sql){
		...
		try{
			List resultList = session.createSQLQuery(sql).addEntity(Train.class).list();
			return resultList;
		}catch(Exception e){
			e.printStackTrace();
			log.info("execute for '" + sql + "' is failed!");
		}finally{
	...
	public List getBySQL(String sql, Class clazz){
		Session session = this.getSession();
		try{
			List resultList = session.createSQLQuery(sql).addEntity(clazz).list();
			return resultList;
		}catch(Exception e){
			e.printStackTrace();
			log.info("execute for '" + sql + "' is failed!");
